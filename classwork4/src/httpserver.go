...
func main() {
	...
	http.HandleFunc("/randtimeout", RandTimeoutHandler)
...
//产生指标的handler
func RandTimeoutHandler(w http.ResponseWriter, r *http.Request){
	glog.Info("Request is coming rand timeout")
	rand.Seed(time.Now().UnixNano())
	timer := metrics.NewTimer()
	defer timer.ObserveTotal()
	delay := rand.Intn(2000)
	time.Sleep(time.Millisecond * time.Duration(delay+100))
	fmt.Fprintln(w, "timeout is ",delay+100)
